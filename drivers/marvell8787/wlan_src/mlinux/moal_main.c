...
#define REV_ID_REG      0x5c
        ...
        case SD8787_A0:
        case SD8787_A1:
            handle->drv_mode->fw_name = SD8787_AX_FW_NAME;//"/lib/fimware/mrvl/sd8787_uapsta.bin";//SD8787_AX_FW_NAME;
            break;
        default:
            break;
        }
    ...
    /*if ((err =
        ...
        printk("request_firmware() failed, error code = %#x\n", err);
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }*/
    
    //handle->firmware->data=sd8787_uapsta;
    //handle->firmware->size=SD8787_UAPSTA_SIZE;
    
    fw.pfw_buf = (t_u8 *) sd8787_uapsta;//handle->firmware->data;
    fw.fw_len = SD8787_UAPSTA_SIZE;//handle->firmware->size;
...
woal_switch_drv_mode(moal_handle * handle, t_u32 mode)
{
    ...
    printk("woal_init_fw/n");
    /* Init FW and HW */
    if (woal_init_fw(handle)) {
    		printk("woal_init_fw error/n");
        PRINTM(MFATAL, "Firmware Init Failed\n");
        goto exit;
    }
    /* Add interfaces */
...
woal_init_module(void)
{
  ...
  //WIFI-------------------------------------------
	/*printk("WIFI power delay:%d %d\n",mydelay1,mydelay2);
	
	//GPG3_2 -->WIFI PWD
	err = gpio_request(S5PV210_GPG3(2), "GPG3");
	if (err) {
		printk(KERN_ERR "failed to request GPG3 for "
			"LVDS chip enable\n");
		return err;
	}

	gpio_direction_output(S5PV210_GPG3(2),0);
	gpio_set_value(S5PV210_GPG3(2), 0);
	
	
	//GPG3_3 -->WIFI RST
	err = gpio_request(S5PV210_GPG3(3), "GPG3");
	if (err) {
		printk(KERN_ERR "failed to request GPG3 for "
			"LVDS chip enable\n");
		return err;
	...
	gpio_set_value(S5PV210_GPG3(3), 0);
	
	
	//GPH1_2 -->WIFI POWER enable
	err = gpio_request(S5PV210_GPH1(2), "GPH1");
	if (err) {
		printk(KERN_ERR "failed to request GPH1 for "
			"LVDS chip enable\n");
		return err;
	}
	...
	gpio_set_value(S5PV210_GPG3(2), 1);
	
	mdelay(mydelay2);
	
	gpio_set_value(S5PV210_GPG3(3), 1);
...
//////////////////////////////////////////////////////
module_param(mydelay1, int, 0);
MODULE_PARM_DESC(mydelay1, "delay time");

module_param(mydelay2, int, 0);
MODULE_PARM_DESC(mydelay2, "delay time");
//////////////////////////////////////////////////////
