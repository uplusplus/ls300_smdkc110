...
print_sys_cfg_max_sta_num_usage(void)
{
    ...
    printf
        ("\nSTA_NUM should be greater than or equal to zero and less than or equal to %d\n",
         MAX_STA_COUNT);
    return;
}
...
apcmd_sys_cfg_eapol_pwk_hsk(int argc, char *argv[])
{
    ...
    /* Send the command */
    ret = uap_ioctl((t_u8 *) cmd_buf, &cmd_len, cmd_len);
    endian_convert_tlv_header_in(timeout_tlv);
    endian_convert_tlv_header_in(retries_tlv);
    tlv_buf = buffer + sizeof(apcmdbuf_sys_configure);
