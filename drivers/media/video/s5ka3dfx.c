...
#define S5K4BA_DRIVER_NAME	"S5KA3DFX"
...
static int cam_power()
{
	...
	if (err)
		printk( "#### failed to request GPB for CAM LDO\n");
	...
	gpio_direction_output(S5PV210_GPH3(5), 0);
	gpio_set_value(S5PV210_GPH3(5), 0);
	
	
	
	//LDO_ON high
	err = gpio_request(S5PV210_GPG3(0), "GPG3");
	if (err)
		printk( "#### failed to request GPH2 for CAM_2V8\n");

	gpio_direction_output(S5PV210_GPG3(0), 1);
	gpio_set_value(S5PV210_GPG3(0), 1);
	
	
	mdelay(1);
	
	//PWD high
	gpio_set_value(S5PV210_GPH3(5), 1);
...
static int s5ka3dfx_read_reg(struct v4l2_subdev *sd, u8 reg, u8 *val)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if (i2c_transfer(client->adapter, msg, 2) != 2) {
		printk("S5KA3DFX I2C read failed reg:%02x\n", reg);
		return -EREMOTEIO;
	}
...
static int s5ka3dfx_write_reg(struct v4l2_subdev *sd, u8 reg, u8 val)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if (i2c_transfer(client->adapter, msg, 1) != 1) {
		printk("S5KA3DFX I2C write failed reg:%02x\n", reg);
		return -EREMOTEIO;
	}
...
static inline int S5KA3DFX_readID(struct v4l2_subdev *sd)
{
	u8 ibuf[5];
	 
	//while(1)
	{
		s5ka3dfx_write_reg(sd,0xef,0x03);//page 0
		s5ka3dfx_write_reg(sd,0x05,0x01);
		mdelay(100);
		
		s5ka3dfx_write_reg(sd,0xef,1);//page 0
		s5ka3dfx_read_reg(sd,0xc5,&ibuf[0]);
		s5ka3dfx_read_reg(sd,0xc6,&ibuf[1]);
		s5ka3dfx_read_reg(sd,0xc7,&ibuf[2]);
		s5ka3dfx_read_reg(sd,0xc8,&ibuf[3]);
		printk("S5KA3DFX ID:[%02x][%02x][%02x][%02x]\n",ibuf[0],ibuf[1],ibuf[2],ibuf[3]);
...
static int s5k4ba_init(struct v4l2_subdev *sd, u32 val)
{
	...
	for (i = 0; i < S5KA3DFX_INIT_REGS; i++) {
		err = s5ka3dfx_write_reg(sd, s5ka3dfx_init_reg[i], \
...
static int s5k4ba_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	...
	if (!pdata->freq)
		state->freq = 12000000;	/* 24MHz default */
	else
