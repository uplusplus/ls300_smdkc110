...
wlan_prog_fw_w_helper(IN pmlan_adapter pmadapter, IN pmlan_fw_image pmfw)
{
    ...
    printk("Downloading FW image (%d bytes)\n", firmwarelen);
    ...
    do {
        ...
        if (ret != MLAN_STATUS_SUCCESS) {
            printk("FW download with helper poll status timeout @ %d\n",
                   offset);
            goto done;
        }
        ...
        if (!len)
           ...
           printk( "FW download failure @ %d, invalid length %d\n",
                   offset, len);
            ret = MLAN_STATUS_FAILURE;
            goto done;
        }
        ...
        if (len & MBIT(0)) {
            ...
            printk( "FW CRC error indicated by the helper:"
                   " len = 0x%04X, txlen = %d\n", len, txlen);
            len &= ~MBIT(0);
