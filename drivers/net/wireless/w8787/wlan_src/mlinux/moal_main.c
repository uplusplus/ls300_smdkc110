 ...
 *  @return        MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
 #define WIFI_POWER  S5PV210_GPH1(2)
 #define WIFI_PDN S5PV210_GPG3(2)
 #define WIFI_RESET S5PV210_GPG3(3)

 void poweron_wifi()
 	{
 	int err;
	err = gpio_request(WIFI_PDN, "GPG3");
	if (err)
		printk(KERN_ERR "#### failed to request GPG3-2 for WIFI PDN\n");

	err = gpio_request(WIFI_RESET, "GPG3");
	if (err)
		printk(KERN_ERR "#### failed to request GPG3-3 for WIFI RESET\n");

	err = gpio_request(WIFI_POWER, "GPH1");
	if (err)
		printk(KERN_ERR "#### failed to request GPH1-2 for WIFI POWER\n");


/*	s3c_gpio_setpull(WIFI_PDN, S3C_GPIO_PULL_NONE);
	gpio_direction_output(WIFI_PDN, 0);


	s3c_gpio_setpull(WIFI_RESET, S3C_GPIO_PULL_NONE);
	gpio_direction_output(WIFI_RESET, 0);

	
    s3c_gpio_setpull(WIFI_POWER, S3C_GPIO_PULL_NONE);
	gpio_direction_output(WIFI_POWER, 0);
	msleep(500);
	gpio_direction_output(WIFI_POWER, 1);	
	msleep(200);
	gpio_direction_output(WIFI_PDN, 1);	
	msleep(200);
...
{  
	printk("delay_poweron_wifi to sleep\n");
	msleep(2000);
	poweron_wifi();
...
woal_init_module(void)
{
	...
	if(ret == MLAN_STATUS_FAILURE)
		printk("woal_bus_register error \n");
