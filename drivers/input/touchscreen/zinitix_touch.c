...
/////////////////////
//for af
#ifdef CONFIG_TOUCH_AUTO_FOCUS
extern void auto_fcous_work();
extern int af_work_status();
int touch_delay=0;
#endif
////////////////////
...
#define	SYSTEM_MAX_X_RESOLUTION	1024//800 //4000	//480
#define	SYSTEM_MAX_Y_RESOLUTION	600//480 //3900	//800
...
#define GPIO_TOUCH_PIN_NUM  	S5PV210_GPH1(2)	// interrupt pin number
#define GPIO_TOUCH_IRQ  	IRQ_EINT10	// interrupt pin IRQ number
...
u32 BUTTON_MAPPING_KEY[MAX_SUPPORTED_BUTTON_NUM]={KEY_BACK, KEY_HOME, KEY_MENU}; //<= you must set key button mapping
...
/*	ret = gpio_request(S5PV210_GPD0(2), "GPD0");
	if (ret) {
		printk(KERN_ERR "failed to request GPD0(2)\n");
		return ret;
	...
	////////////////////////////////////////////
		...
		////////////////////////////////////
	//i2c.client->irq=IRQ_EINT11;

		printk("ilitek_i2c setup gpio \n");
		
		gpio_request(S5PV210_GPH1(3), "GPH1");
...
*/
			...
			if(reported == true)
			{
				...
				for(i=0; i< touch_dev->cap_info.multi_fingers; i++)
				{
					...
					if(zinitix_bit_test(touch_dev->reported_touch_info.coord[i].sub_status, SUB_BIT_EXIST))			
					{
	//					printk("finger [%02d] x = %d, y = %d \r\n", i, touch_dev->reported_touch_info.coord[i].x, touch_dev->reported_touch_info.coord[i].y);
			...
			for(i=0; i< touch_dev->cap_info.multi_fingers; i++)
			{
					...
					|| zinitix_bit_test(touch_dev->touch_info.coord[i].sub_status, SUB_BIT_EXIST))				
				{
					x = touch_dev->touch_info.coord[i].x;
					y = touch_dev->touch_info.coord[i].y;
			//		printk("SUB_BIT_DOWN | SUB_BIT_MOVE | SUB_BIT_EXIST\n");
			        //////////////////////
	//for af
#ifdef CONFIG_TOUCH_AUTO_FOCUS
					if(af_work_status() && x < 730 && y < 730)
					{
						//printk("camera run & touch screen!\n");
						//if(touch_delay==30)
							auto_fcous_work();
					...
					input_report_abs(touch_dev->input_dev, ABS_MT_POSITION_X, x);
				        input_report_abs(touch_dev->input_dev, ABS_MT_POSITION_Y, 600-y);
//					input_report_abs(touch_dev->input_dev, ABS_MT_POSITION_X, touch_dev->reported_touch_info.coord[i].x);
//					input_report_abs(touch_dev->input_dev, ABS_MT_POSITION_Y, touch_dev->reported_touch_info.coord[i].y);
...
//#endif
{
	...
	switch(cmd)
	{
		...
		case TOUCH_IOCTL_VARIFY_UPGRADE_SIZE:
		{
			...
			printk(KERN_INFO "firmware size = %d need size = %d\r\n", sz,misc_touch_dev->cap_info.chip_fw_size);
...
//by fanwb
//	touch_dev->cap_info.Orientation = TOUCH_XY_SWAP;
