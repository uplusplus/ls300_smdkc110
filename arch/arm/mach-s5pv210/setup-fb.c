...
int s3cfb_lcd_on(struct platform_device *pdev)
{
	...
	//GPH2_4 -->LCD enable
	err = gpio_request(S5PV210_GPH2(4), "GPD0");
	if (err) {
		printk(KERN_ERR "failed to request GPH0 for "
			"LVDS chip enable\n");
		return err;
	...
	s3c_gpio_cfgpin(S5PV210_GPH2(4), 0x1<<16);
	...
	gpio_direction_input(S5PV210_GPG3(2));
	...
	gpio_direction_input(S5PV210_GPG3(3));
...
int s3cfb_lcd_on(struct platform_device *pdev)
{
	...
	gpio_set_value(S5PV210_GPG3(4), 1);
	gpio_free(S5PV210_GPG3(4));
